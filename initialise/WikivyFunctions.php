...
class WikivyFunctions
{
	...
	public static function getLocalDatabases(): array
	{
		...
		$databases ??= [
			...self::readDbListFile('databases'), ...self::readDbListFile('deleted'),
		];
	...
	public static function readDbListFile(
		...
		if ($wgDatabaseClustersMaintenance) {
			$databases = array_filter($databases, static function (array $data, string $key): true {
				...
				foreach (self::SUFFIXES as $suffix => $domains) {
					if (str_ends_with($database, $suffix)) {
						$defaultServer = $databases['d']
							??
							self::SUFFIXES[$suffix][array_search(self::DEFAULT_SERVER[$realm], self::SUFFIXES[$suffix],
								true)];
	...
	{
		return str_ends_with($this->server, '.wikivy.dev');
	}
	...
	public static function getDatabaseClusters(): array
	{
		...
		return array_combine(array_keys($databases), array_column($databases, 'c'));
	}
	...
	public static function getSiteNames(): array
	{
		...
		$siteNames = array_combine(array_keys($databases), array_column($databases, 's')) ?: [];
	...
	public static function getConfigGlobals(): array
	{
		...
		$confActualMtime = max(filemtime(__DIR__.'/../LocalSettings.php'),
			filemtime(__DIR__.'/../ManageWikiExtensions.php'), filemtime(__DIR__.'/../ManageWikiNamespaces.php'),
			filemtime(__DIR__.'/../ManageWikiSettings.php'), filemtime(MW_INSTALL_PATH.'/includes/Defines.php'),
			@filemtime(self::CACHE_DIRECTORY."/$wgDBname.php"));

		static $globals = null;
		$globals ??= self::readFromCache(self::CACHE_DIRECTORY."/$confCacheFileName", 'globals', $confActualMtime);

		if (!$globals) {
			// Merge ManageWiki cache into wgConf
			$wgConf->settings = [
				...$wgConf->settings, ...self::getManageWikiConfigCache()
			];
			...
			$confCacheObject = [
				'mtime' => $confActualMtime, 'globals' => $globals, 'extensions' => self::$activeExtensions,
			];
	...
	public static function getConfigForCaching(): array
	{
		...
		$wikiTags = [
			...preg_filter('/^/', 'ext-', str_replace(' ', '', self::$activeExtensions)), ...$wikiTags,
		];
	...
	public static function getManageWikiConfigCache(): array
	{
		...
		$settings['cwInactive']['default'] = $states['inactive'] === 'exempt'
			? 'exempt'
			: (bool) ($states['inactive']
				??
				false);
	...
	public static function handleDisabledExtensions(): void
	{
		...
		foreach (static::$disabledExtensions as $name => $reason) {
			$wgManageWikiExtensions[$name]['help'] = '<b>Note</b>: This extension has been globally disabled. The following reason was given: '.
				$reason;
		...
		if ($cacheData === false || $cacheData === []) {
			...
			if (self::getRealm($wgDBname) !== 'default') {
				$wgConf->siteParamsCallback = static fn(): array => [
					'suffix' => null, 'lang' => 'en', 'tags' => [self::getRealm()], 'params' => [],
				];
			}
	...
	{
		return MediaWikiServices::getInstance()->getConnectionProvider()->getReplicaDatabase($databaseName);
	}
		...
		foreach ($allWikis as $wiki) {
			if ((int) $wiki->wiki_deleted === 1) {
				$deletedList[$wiki->wiki_dbname] = [
					's' => $wiki->wiki_sitename, 'c' => $wiki->wiki_dbcluster,
				];
			} else {
				...
				$primaryDomain = ($extraData['primary-domain'] ?? null)
					?: self::DEFAULT_SERVER[self::getRealm($wiki->wiki_dbname)];
				$wikiVersion = ($extraData['mediawiki-version'] ?? null)
					?: self::MEDIAWIKI_VERSIONS[self::getDefaultMediaWikiVersion()];

				$combiList[$wiki->wiki_dbname] = [
					's' => $wiki->wiki_sitename, 'c' => $wiki->wiki_dbcluster, 'd' => $primaryDomain,
					'v' => $wikiVersion,
				];
	...
	public static function onManageWikiCoreAddFormFields(
		...
		$versions = array_unique(array_filter(self::MEDIAWIKI_VERSIONS,
			static fn(string $version): bool => $mwVersion === $version || is_dir(self::MEDIAWIKI_DIRECTORY.$version)));
	...
	public static function onManageWikiCoreFormSubmission(
		...
		if ($mediawikiVersion !== $version && is_dir(self::MEDIAWIKI_DIRECTORY.$mediawikiVersion)) {
			$mwCore->setExtraFieldData('mediawiki-version', $mediawikiVersion, default: $version);
		}

		$domain = self::getPrimaryDomain($dbname);
		$primaryDomain = $formData['primary-domain'] ?? $domain;
		if ($primaryDomain !== $domain) {
			$mwCore->setExtraFieldData('primary-domain', $primaryDomain, default: $domain);
		}
		...
		if ($formData['mainpage-is-domain-root'] !== $mainPageIsDomainRoot) {
			...
			$mwCore->trackChange('mainpage-is-domain-root', $mainPageIsDomainRoot,
